...
Figure *figure(const std::string &title_="", const std::vector<int> position={500, 400});
...
signals:
  ...
  void set_xlim_signal(double min, double max);
  void set_ylim_signal(double min, double max);
...
private slots:
  ...
  void set_xlim_slot(double min, double max);
  void set_ylim_slot(double min, double max);
...
public:
  ...
  static int fig_id; // the id to be assigned on the next created figure
  ...
  static bool isInitialized() { return QtPlot::initialized; }

signals:

  void figureSignal(Figure **);
  void closeSignal(Figure *);

  void terminateSignal();

protected:
  friend Figure;
  friend void closeAll();
  friend void drawnow();

  static void addFigure(Figure *fig);
  static void removeFigure(Figure *fig);

private slots:

  void figureSlot(Figure **);
  void closeSlot(Figure *);

private:
  static std::map<int, Figure *> fig_map;
